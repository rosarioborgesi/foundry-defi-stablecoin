...
contract DSCEngine is ReentrancyGuard {
    ...
    constructor(address[] memory tokenAddresses, address[] memory priceFeedAddresses, address dscAddress) {
        ...
        // USD Price Feeds. For example ETH/USD, BTC/USD, etc.
        ...
        external
    {
        ...
        // we don't need _revertIfHealthFactorIsBroken because redeemCollateral already does that
    }
    ...
    function burnDsc(uint256 amount) public moreThanZero(amount) {
        ...
        _revertIfHealthFactorIsBroken(msg.sender); // I don't think we need this
    }
        ...
        nonReentrant
    {
        ...
        // debtToCover = $100
        // We are going to pay $100 of DSC but how much ETH is that? --> 0.05 ETH
        ...
        _burnDsc(debtToCover, user, msg.sender); // Who is calling this function (msg.sender) is going to liquidate the user so is paying back the minted DSC
    ...
    function _redeemCollateral(address from, address to, address tokenCollateralAddress, uint256 amountCollateral)
        private
    {
        // If they have 100 and they try to pull out 1000 this will revert
    ...
    function getTokenAmountFromUsd(address token, uint256 usdAmountInWei) public view returns (uint256) {
        // We need the price of ETH (or the token)
        // We have the rate $/ETH now we have to figure out how much ETH is usdAmountInWei
        // For instance the rate $/ETH is 2000$/ETH so $1000 is 0.5 ETH
        AggregatorV3Interface priceFeed = AggregatorV3Interface(s_priceFeeds[token]);
        (, int256 price,,,) = priceFeed.latestRoundData();
        // ($10e18 * 1e18) / ($2000e8 * 1e10) = 0.005 ETH
        return (usdAmountInWei * PRECISION) / (uint256(price) * ADDITIONAL_FEED_PRECISION);
    }

    function getAccountCollateralValue(address user) public view returns (uint256 totalCollateralValueInUsd) {
        // loop through each collateral token, get the amount they have deposited, amd map it to
    ...
    function getUsdValue(address token, uint256 amount) public view returns (uint256) {
        ...
        (, int256 price,,,) = priceFeed.latestRoundData();
        // 1 ETH = $1000
        // The returned value from CL will be 1000 * 1e8 because the ETH/USD and BTC/USD price feed decimals are 8
        return (uint256(price) * ADDITIONAL_FEED_PRECISION * amount) / PRECISION;
    }
